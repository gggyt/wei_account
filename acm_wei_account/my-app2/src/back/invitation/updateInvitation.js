...
class UpdateInvatition extends React.Component {
    ...
    fetch(GetInvitationDetail, {
      ...
      data => {
       ...
       // window.alert(data.code);
        if (data.code==0) {
          this.setState({title: data.resultBean.invitationTitle});
          this.setState({editorContent: this.state.editor.txt.html(data.resultBean.invitationBody)});
        } else {
  ...
  updateCompetition() {
    ...
    fetch(UpdateInvitation,{   //Fetch方法
            ...
            body: 'invitationId='+this.props.match.params.id+'&invitationTitle='+this.state.title+'&invitationBody='+encodeURI(this.state.editorContent)
    ...
    return (
      ...
      <div className="col-md-9">
        ...
        <div className="inputTitle">
          <Input size="small" placeholder="帖子名称" value={this.state.title} style={{height:30}} onChange={this.titleChange}/>
        </div>
        <br/>
        <div className="inputTitle">
...
export default UpdateInvatition;