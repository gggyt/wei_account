...
class AddInvitationShow extends React.Component {
  ...
  saveCompetition() {
    ...
    fetch(AddInvitation,{   //Fetch方法
      ...
      body: 'invitationTitle='+this.state.title+'&invitationBody='+encodeURI(this.state.editorContent)
  ...
  updateCompetition() {
    ...
    fetch(UpdateInvitation,{   //Fetch方法
      ...
      body: 'invitationId='+this.state.id+'&invitationTitle='+this.state.title+'&invitationBody='+encodeURI(this.state.editorContent)
    }).then(res => res.json()).then(
      data => {
        if(data.code==0) {
