...
class UpdateProblemDetail extends React.Component{
  constructor(props){
    super(props);
    this.state={
      invitationTitle:'',
  ...
  add() {
    ...
    fetch(UpdateProblemUrl,{   //Fetch方法
      ...
      body: 'problemId='+this.props.match.params.id+'&problemTitle='+this.state.invitationTitle+'&problemBody='+encodeURI(this.state.invitationBody)
        ...
        //window.alert('code'+data.code);
          if(data.code==0) {
            ...
            this.props.history.push('/mobile/problemDetail/'+this.props.match.params.id);
        }
        else {
          setTimeout(hide)
  ...
  getDetail() {
    fetch(DetailProblemUrl,{   //Fetch方法
      ...
      body: 'problemId='+this.props.match.params.id
        ...
        //window.alert('code'+data.code);
          if(data.code==0) {
            this.setState({invitationTitle: data.resultBean.problemTitle});
          this.setState({invitationBody: this.state.editor.txt.html(data.resultBean.problemBody)});
            this.setState({visible: false});
        }
        else {
          Toast.faile(data.msg);
