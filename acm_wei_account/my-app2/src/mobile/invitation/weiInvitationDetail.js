...
import {NewsInvitation} from '../../config/router.js';
...
class InvitationView extends React.Component{
		...
		fetch(NewsInvitation,{   //Fetch方法
            ...
            data => {
                ...
                if(data.code==0) {
                	...
                	this.setState({invitationId: data.resultBean.invitationId});
                }
                else {
                	message.error(data.msg);
                }
	...
	render() {
		...
		<div>
			...
			<div>
     	 		{this.state.invitationId!=''?<CommentList invitationId={this.state.invitationId} />:<p></p>}
