...
class CommentView extends React.Component{
	constructor(props){
		...
		this.state = {
	      ...
	      pageSize: 5,
	      commentAll:[],
	      p_commentId:this.props.commentId,
		}
	...
	render(){
		...
		if (this.props.commentId!=this.state.p_commentId){
			this.setState({p_commentId: this.props.commentId}, () => this.getComment());
		}
		return(
		...
		fetch(GetInvitationDetail,{   //Fetch方法
            ...
            data => {
                ...
                if(data.code==0) {
                	...
                	this.setState({createUserId:data.resultBean.createUserId});
                }
                else {
                	message.error(data.msg);
                }
		...
		return(
			...
			<div className="backCD">
				...
				<div>
					<Meta
		            avatar={ 
			            this.state.image!=''?<a><Link to={'/mobile/userIndex/'+this.state.createUserId}><Avatar size={50} src={this.state.image} /></Link></a>:
			            <a><Link to={'/mobile/userIndex/'+this.state.createUserId}><Avatar size={50} style={{ backgroundColor: '#f56a00', verticalAlign: 'middle' }} >
             			{this.state.createUser}</Avatar></Link></a>
			        }
		            title={this.state.invitationTitle}
		            description={ <div><span>{this.state.createUser}发表于{this.state.createDate}</span>
		            <span className="floatR"><Divider type="vertical" />阅读{this.state.readNum}</span> </div>}/>
		        </div>
		        <hr/>
		        <div>
